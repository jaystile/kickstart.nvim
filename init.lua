...
Python setup video: https://youtu.be/IobijoroGE0?si=Zvhtv2SyolGsESBF
...
if vim.g.vscode then
  -- VSCode has a lot of things that the plugins do, so we want VSCode do what it is going to do.
  -- do nothign (for now)
else
  ...
  vim.keymap.set('t', '<Esc>', '<C-\\><C-n>', { desc = 'Use escape to exit terminal insert mode' })
      ...
      config = function()
        ...
        vim.api.nvim_create_autocmd('LspAttach', {
          ...
          callback = function(event)
            ...
            map('<leader>br', vim.lsp.buf.rename, '[B]uf [R]ename')

            -- Execute a code action, usually your cursor needs to be on top of an error
            -- or a suggestion from your LSP for this to activate.
            map('<leader>bc', vim.lsp.buf.code_action, '[B]uf [C]ode Action', { 'n', 'x' })

            -- WARN: This is not Goto Definition, this is Goto Declaration.
            --  For example, in C this would take you to the header.
            map('<leader>bd', vim.lsp.buf.declaration, '[B]uf [D]eclaration')

            -- Display the hover text
            map('<leader>bh', vim.lsp.buf.declaration, '[B]uf [H]over')

            -- Display the signature
            map('<leader>bs', vim.lsp.buf.signature_help, '[B]uf [S]ignature')
        ...
        local servers = {
          ...
          -- ts_ls = {},
          --
          --pylsp instead of pyright to address memory issues
          pylsp = {
            settings = {
              pylsp = {
                plugins = {
    ...
    { -- Highlight, edit, and navigate code
      ...
      opts = {
        ensure_installed = {
          ...
          'eex', -- elixir development
          'heex', -- elixir development
          'html',
          'lua',
          'luadoc',
          'markdown',
